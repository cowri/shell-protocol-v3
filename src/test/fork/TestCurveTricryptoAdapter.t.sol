...
contract TestCurveTricryptoAdapter is Test {
    ...
    address lpWallet = 0x2D84a7364d4e8D96bB4e403a0875BA26C455f04f; // Tricrypto LP whale
    ...
    function testSwapEth(bool toggle, bool isInput, uint256 amount, uint256 unwrapFee) public {
        ...
        if (isInput) {
            ...
            interactions[1] = Interaction({ interactionTypeAndAddress: _fetchInteractionId(outputAddress, uint256(InteractionType.UnwrapErc20)), inputToken: 0, outputToken: 0, specifiedAmount: type(uint256).max, metadata: bytes32(0) });
            ...
            interactions[2] = Interaction({ interactionTypeAndAddress: _fetchInteractionId(outputAddress, uint256(InteractionType.UnwrapEther)), inputToken: 0, outputToken: 0, specifiedAmount: type(uint256).max, metadata: bytes32(0) });
