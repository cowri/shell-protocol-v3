...
contract MockPrimitive is IOceanPrimitive {
    ...
    function computeOutputAmount(uint256 inputToken, uint256 outputToken, uint256 inputAmount, address userAddress, bytes32 metadata) external override onlyOcean returns (uint256 outputAmount) {
        ...
        (uint256[] memory burnIds, uint256[] memory burnAmounts, uint256[] memory mintIds, uint256[] memory mintAmounts) = IOceanInteractions(ocean).doMultipleInteractions(interactions, ids);
