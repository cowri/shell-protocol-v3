...
contract Curve2PoolQuery is IPoolQuery {
    ICurveQuery pool;
    ICurveQuery statelessPool;
    ...
    function swapGivenInputAmount(uint256 xBalance, uint256 yBalance, uint256 inputAmount, SpecifiedToken inputToken) public view returns (uint256 outputAmount) {
        ...
        uint256 rawOutputAmount = statelessPool.get_dy([rawXBalance, rawYBalance], inputID, outputID, rawInputAmount);
    ...
    function depositGivenInputAmount(uint256 xBalance, uint256 yBalance, uint256 totalSupply, uint256 depositAmount, SpecifiedToken depositToken) public view returns (uint256 mintAmount) {
        ...
        uint256 rawOutputAmount = statelessPool.calc_token_amount([rawXBalance, rawYBalance], rawTotalSupply, inputAmounts, true);
    ...
    function withdrawGivenInputAmount(uint256 xBalance, uint256 yBalance, uint256 totalSupply, uint256 burnAmount, SpecifiedToken withdrawnToken) public view returns (uint256 withdrawnAmount) {
        ...
        uint256 rawOutputAmount = statelessPool.calc_withdraw_one_coin([rawXBalance, rawYBalance], rawTotalSupply, rawInputAmount, outputID);
...
interface ICurveQuery {
    ...
    function get_dy(uint256[2] memory balances, int128 i, int128 j, uint256 dx) external view returns (uint256);

    function calc_token_amount(uint256[2] memory _amounts, bool _is_deposit) external view returns (uint256);

    function calc_token_amount(uint256[2] memory balances, uint256 totalSupply, uint256[2] memory _amounts, bool _is_deposit) external view returns (uint256);

    function calc_withdraw_one_coin(uint256 _burn_amount, int128 i) external view returns (uint256);

    function calc_withdraw_one_coin(uint256[2] memory balances, uint256 totalSupply, uint256 _burn_amount, int128 i) external view returns (uint256);
}
